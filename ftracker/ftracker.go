...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	caloriesPerPeriod := (runningCaloriesMeanSpeedMultiplier * meanSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm
	spentCalories := caloriesPerPeriod * duration * minInH
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	caloriesPerPeriod := walkingCaloriesWeightMultiplier*weight + math.Pow(averageSpeed, 2)/(height/cmInM)*walkingSpeedHeightMultiplier*weight
	spentCalories := caloriesPerPeriod * duration * minInH
	return spentCalories
}
...
func SwimmingSpentCalories(lenghtPool, countPool int, duration, weight float64) float64 {
	averageSpeed := swimmingMeanSpeed(lenghtPool, countPool, duration)
	caloriesPerPeriod := (averageSpeed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight
	spentCalories := caloriesPerPeriod * duration
	return spentCalories
}
